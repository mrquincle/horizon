...
echo "The url used for the mediabox is 10.0.0.38, yours might very well be different!"
mediabox_url=http://10.0.0.38/cgi-bin

echo "We store the cookie in cookie.txt, it has a sessionID"
cookie=cookie.txt

headers='-H "User-Agent: Mozilla/5.0" -H "Connection: keep-alive"'
...
if [[ "$skip_token" == "no" ]]; then
	echo "We will now do a curl request, I might do something wrong here, so you might need also to login to your browser"
	curl $all_headers --cookie-jar $cookie --data "page=login" --data "username=admin" --data "password=admin" "$mediabox_url/sendResult.cgi?section=login"
...
echo Command: $all_headers --data "page=..." --data "field=$payload" 
